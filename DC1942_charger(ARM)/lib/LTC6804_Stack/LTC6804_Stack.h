...
#ifndef LTC6804_STACK_H
#define LTC6804_STACK_H
...
{
	public:
		LTC6804_Stack( uint16_t chipSel );
		~LTC6804_Stack( void );
		void setup( void );
...
bool LTC6804_Stack::get_all_cell_voltages( const uint8_t icCount, const uint8_t* const icAddresses, CV_T cellVoltages[][LTC6804_NUM_CELLS] )
{
...
template <typename CV_T>
bool LTC6804_Stack::get_cell_voltages( const int8_t addr, CV_T *const cellVoltages )
{
...
bool LTC6804_Stack::get_all_auxillary( const uint8_t icCount, const uint8_t* const icAddresses, AUX_T gpioVoltages[][LTC6804_NUM_GPIOS], AUX_T* const ref2Voltages )
{
	for( uint8_t icIndex = 0; icIndex < icCount; icIndex++ )
	{
...
template <typename AUX_T>
bool LTC6804_Stack::get_auxillary( const int8_t addr, AUX_T* const gpioVoltages, AUX_T* const ref2VoltagePtr )
{
...
bool LTC6804_Stack::get_all_status( const uint8_t icCount, const uint8_t* const icAddresses, STAT_T socVoltages[], STAT_T internalTemps[], STAT_T analogSupplyVoltages[], STAT_T digitalSupplyVoltages[], uint16_t uvFlags[], uint16_t ovFlags[], uint8_t muxFails[], uint8_t thermalShutdowns[] )
{
	for( uint8_t icIndex = 0; icIndex < icCount; icIndex++ )
	{
...
// recieved PECs and requesting retransmission if data is corrupted.
template <typename STAT_T>
bool LTC6804_Stack::get_status( const int8_t addr, STAT_T* const socVoltagePtr, STAT_T* const internalTempPtr, STAT_T* const analogSupplyVoltagePtr, STAT_T* const digitalSupplyVoltagePtr, uint16_t* const uvFlagsPtr, uint16_t* const ovFlagsPtr, uint8_t* const muxFailPtr, uint8_t* const thermalShutdownPtr )
{
