...
#define LTC6804_ADDR_BIT 0x8000
...
#define LTC6804_CC_ADDR_LSB 11
// Argument placement in command words
const uint8_t LTC6804_CC_ARG_LSBS[] PROGMEM = {
  7, 4, 0, 6, 5, 0, 0
};
// Argument values
const uint8_t LTC6804_CC_ARG_VALUES[][LTC6804_MAX_NUM_CC_ARG_VALUES] PROGMEM = {
	{1, 2, 3}, 				// ADC mode
...
uint8_t LTC6804_Stack::_numStacks = 0;
uint16_t* LTC6804_Stack::_chipSels;
bool* LTC6804_Stack::_timerMasks;
...
ISR (TIMER1_OVF_vect)
{
	LTC6804_Stack::timer_overflow_isr();
  // Interrupt flag is cleared by hardware
} // end timer1 overflow ISR


// Constructor
LTC6804_Stack::LTC6804_Stack( uint16_t chipSel )
{
	_chipSel = chipSel;
	_stackId = _numStacks;
...
// Destructor
LTC6804_Stack::~LTC6804_Stack( void )
{
	--_numStacks;
	_chipSels = (uint16_t*) realloc( _chipSels, _numStacks * sizeof(uint16_t) );
	_timerMasks = (bool*) realloc( _timerMasks, _numStacks * sizeof(bool) );
...
bool LTC6804_Stack::config_all( const uint8_t icCount, const uint8_t* const icAddresses, const uint16_t* const cfgGrp )
{
...
bool LTC6804_Stack::config( const int8_t addr, const uint16_t* const cfgGrp )
{
...
bool LTC6804_Stack::verify_all_config( const uint8_t icCount, const uint8_t* const icAddresses, const uint16_t* const cfgGrp, bool* const flags )
{
	bool configOk = true;
...
bool LTC6804_Stack::verify_config( const int8_t addr, const uint16_t* const cfgGrp )
{
	uint16_t	readBackPec, cmpPec;
	uint16_t	readBackCfgGrp[LTC6804_NUM_CONFIG_WORDS],
						cfgGrpCopy[LTC6804_NUM_CONFIG_WORDS];
...
// data is corrupted.
bool LTC6804_Stack::get_raw_config( const int8_t addr, uint16_t* const configData )
{
...
bool LTC6804_Stack::get_raw_cell_voltages( const int8_t addr, uint16_t* const cvData )
{
...
// array whose starting address is given. Broadcast reads are not supported by this function. Implements
// a data integrity check by verifying recieved PECs and requesting retransmission if data is corrupted.
bool LTC6804_Stack::get_raw_auxillary( const int8_t addr, uint16_t* const auxData )
{
...
// array whose starting address is given. Broadcast reads are not supported by this function. Implements
// a data integrity check by verifying recieved PECs and requesting retransmission if data is corrupted.
bool LTC6804_Stack::get_raw_status( const int8_t addr, uint16_t* const statData )
{
...
// to zero.
void LTC6804_Stack::prepare_config( uint16_t* const cfgGrp, const uint8_t gpioPulldownCtl, const uint8_t ref, const uint8_t adcOpt, const uint16_t uvThreshold, const uint16_t ovThreshold, const uint16_t cellDischargeCtl, const uint8_t dischargeTimeout )
{
...
void LTC6804_Stack::extract_cell_voltages( const uint16_t* const regData, float* const cvData )
{
...
// also specified.
void LTC6804_Stack::extract_cell_voltages( const uint16_t* const regData, uint16_t* const cvData )
{
...
void LTC6804_Stack::extract_auxillary( const uint16_t* const regData, float* const gpioVoltages, float* const ref2Voltage )
{
...
void LTC6804_Stack::extract_auxillary( const uint16_t* const regData, uint16_t* const gpioVoltages, uint16_t* const ref2Voltage )
{
...
void LTC6804_Stack::extract_status( const uint16_t* const regData, float* const socVoltage, float* const internalTemp, float* const analogSupplyVoltage, float* const digitalSupplyVoltage, uint16_t* const uvFlags, uint16_t* const ovFlags, uint8_t* const muxFail, uint8_t* const thermalShutdown )
{
...
// starting address is specified, into the floating-point and unsigned integer
// varaibles whose memory addresses are specified.
void LTC6804_Stack::extract_status( const uint16_t* const regData, uint16_t* const socVoltage, uint16_t* const internalTemp, uint16_t* const analogSupplyVoltage, uint16_t* const digitalSupplyVoltage, uint16_t* const uvFlags, uint16_t* const ovFlags, uint8_t* const muxFail, uint8_t* const thermalShutdown )
{
...
void LTC6804_Stack::clear_status( const int8_t addr )
{
...
void LTC6804_Stack::start_cell_voltage_adc( const int8_t addr, const uint8_t mode, const uint8_t dischargePermit, const uint8_t cellChannel )
{
...
// the ADAX command.
void LTC6804_Stack::start_auxillary_adc( const int8_t addr, const uint8_t mode, const uint8_t auxChannel )
{
...
// the ADAX command.
void LTC6804_Stack::start_status_adc( const int8_t addr, const uint8_t mode, const uint8_t statusChannel )
{
...
// '6804 IC whose address is supplied. Specify addr as BROADCAST or -1 to broadcast the
// command to all ICs.
void LTC6804_Stack::start_diagn( const int8_t addr )
{
...
// to all ICs.
void LTC6804_Stack::write_config( const int8_t addr, const uint16_t* const regDataPtr )
{
...
// pointer points.
void LTC6804_Stack::read_config( const uint8_t addr, uint16_t* const regDataPtr, uint16_t* const dataPecPtr )
{
...
void LTC6804_Stack::read_cell_voltages( const uint8_t addr, uint16_t* const regDataPtr, uint16_t* const dataPecsPtr )
{
...
// for each register group into another array. The target IC is specified
// by its isoSPI address.
void LTC6804_Stack::read_auxillary( const uint8_t addr, uint16_t* const regDataPtr, uint16_t* const dataPecsPtr )
{
...
// for each register group into another array. The target IC is specified
// by its isoSPI address.
void LTC6804_Stack::read_status( const uint8_t addr, uint16_t* const regDataPtr, uint16_t* const dataPecsPtr )
{
...
// the SPI bus.
void LTC6804_Stack::write_cmd( const uint16_t cmd, const uint16_t cmdPec )
{
  begin_transaction();
  // Wakeup isoSPI port; assume core kept awake by keep alive timer
  wakeup_iso_spi();
...
void LTC6804_Stack::read_regs( uint16_t* const regDataPtr, uint16_t* const dataPecsPtr, const uint16_t* const readCmdsPtr, const uint16_t* const cmdPecsPtr, const uint8_t numRegs, const uint8_t numWordsPerReg )
{
  begin_transaction();
  // Wakeup isoSPI port; assume core kept awake by keep alive timer
  wakeup_iso_spi();
...
void LTC6804_Stack::begin_transaction(void)
{
...
void LTC6804_Stack::end_transaction(void)
{
  SPI.endTransaction();
...
void LTC6804_Stack::mask_keep_alive_timer_int( void )
{
	_timerMasks[_stackId] = true;
} // end mask_keep_alive_timer_int


void LTC6804_Stack::unmask_keep_alive_timer_int( void )
{
	_timerMasks[_stackId] = false;
...
// the SPI port.
void LTC6804_Stack::spi_write_buf( const uint16_t* buf, uint8_t len )
{
...
void LTC6804_Stack::spi_read_buf( uint16_t* const buf, uint8_t len )
{
  for( uint8_t i = 0; i < len; i++ )
  {
...
void LTC6804_Stack::wakeup_iso_spi( void )
{
  digitalWrite( _chipSel, LOW );
...
void LTC6804_Stack::wakeup_core( void )
{
  digitalWrite( _chipSel, LOW );
...
uint16_t LTC6804_Stack::prepare_command( const uint16_t code )
{
...
void LTC6804_Stack::bind_address( uint16_t* const cmdPtr, const uint8_t addr )
{
  *cmdPtr |= LTC6804_ADDR_BIT | (addr << LTC6804_CC_ADDR_LSB);
} // end bind_address
...
void LTC6804_Stack::bind_address_array( uint16_t* const cmdPtr, uint8_t len, const uint8_t addr )
{
  for( uint8_t i = 0; i < len; i++ )
  {
...
// value).
void LTC6804_Stack::bind_argument( uint16_t* const cmdPtr, const uint8_t argId, const uint8_t valId )
{
  *cmdPtr |= pgm_read_byte( &(LTC6804_CC_ARG_VALUES[argId][valId]) ) << pgm_read_byte( &LTC6804_CC_ARG_LSBS[argId] );
} // end bind_argument
...
void LTC6804_Stack::bind_data( uint16_t* const regGrp, const uint8_t dataId, const uint16_t valId )
{
...
// on the specified register-data identifier (an 8-bit value).
uint16_t LTC6804_Stack::strip_data( const uint16_t* const regGrp, const uint8_t dataId )
{
  // Lookup positioning information for the data item corresponding to the supplied id
  const uint8_t NUM_DATA_SEGMENTS = pgm_read_byte( &(LTC6804_REG_DATA_POS[dataId][REG_DATA_NUM_SEGMENTS_INDEX]) ),
                DATA_SEGMENT_SIZE = pgm_read_byte( &(LTC6804_REG_DATA_POS[dataId][REG_DATA_SEGMENT_SIZE_INDEX]) );
...
void LTC6804_Stack::setup_keep_alive_timer( void )
{
  stop_keep_alive_timer();
  ...
  //TIMSK1 |= bit( TOIE1 );
 // attachInterrupt(void (*myhandler(interruptA));
} // end setup_keep_alive_timer


void LTC6804_Stack::clear_keep_alive_timer( void )
{
  //TCNT1 = 0;
} // end clear_keep_alive_timer


void LTC6804_Stack::start_keep_alive_timer( void )
{
...
void LTC6804_Stack::stop_keep_alive_timer( void )
{
...
void LTC6804_Stack::timer_overflow_isr( void )
{
...
void LTC6804_Stack::adc_delay( const uint8_t conversionType, const uint8_t adcOpt, const uint8_t adcMode, const uint8_t channel )
{
...
uint32_t LTC6804_Stack::get_adc_delay_us( const uint8_t conversionType, const uint8_t adcOpt, const uint8_t adcMode, const uint8_t channel )
{
	return pgm_read_dword( &(LTC6804_ADC_TIMES_US[conversionType][adcOpt][adcMode][channel]) );
} // end get_adc_delay_us


bool LTC6804_Stack::digital_write( const int8_t addr, uint8_t gpioStates )
{
...
void LTC6804_Stack::bind_gpio_state( uint8_t* gpioStates, uint8_t gpio, uint8_t state )
{
	if( state == HIGH )
	{
